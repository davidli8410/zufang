...
elseif($_REQUEST['act'] == 'postok')
{
	...
	$fax       = $_POST['address'] ? htmlspecialchars($_POST['fax']) : '';
	$address   = $_POST['address'] ? htmlspecialchars($_POST['address']) : '';
	$mappoint  = $_POST['mappoint'] ? htmlspecialchars($_POST['mappoint']) : '';
    ...
    $sql = "insert into {$table}com (userid,catid,areaid,comname,keywords,description,linkman,email,fax,qq,phone,postdate,mappoint,address,hours,is_check,introduce) 
	values ('$_userid','$catid','$areaid','$comname','$keywords','$description','$linkman','$email','$fax','$qq','$phone','$postdate','$mappoint','$address','$hours','$is_check','$introduce')";
    $res = $db->query($sql);
	$id = $db->insert_id();
