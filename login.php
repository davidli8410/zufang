...
switch($act)
{
	...
	case 'act_login':
		...
		$username = $_POST['username_or_email'] ? htmlspecialchars_deep(trim($_POST['username_or_email'])) : '';
		...
		if(login($username,$md5_password)) {
			echo "{\"login_status\": \"SCMD_LOGIN_SUCCEED\"}";
		} else {
			echo "{\"login_status\": \"SCMD_LOGIN_WRONG\"}";
		}
	break;
	...
	case 'act_register':
		...
		if(empty($username))showmsg("用户名不能为空");
		if(!check_name($username))showmsg("用户名仅可使用汉字、字母、数字、中划线或下划线。");
		if(empty($password))showmsg("密码不能为空");
	...
	case 'act_modify':
		$phone    = $_POST['phone'] ? htmlspecialchars_deep($_POST['phone']) : '';
		$qq       = $_POST['qq'] ? intval($_POST['qq']) : '';
		$address  = $_POST['address'] ? htmlspecialchars_deep($_POST['address']) : '';
		$mappoint = $_POST['mappoint'] ? htmlspecialchars_deep($_POST['mappoint']) : '';
		$userid   = $_SESSION['userid'];
		$username = $_SESSION['username'] ? htmlspecialchars_deep($_SESSION['username']) : '';
		$email    = htmlspecialchars_deep($_POST['email']);
		if(!is_email($email))showmsg('Email 格式有误');
	...
	case 'send':
		$paycenter = htmlspecialchars_deep($_POST['paycenter']);
		$contactname = htmlspecialchars_deep($_POST['contactname']);
		$telephone = htmlspecialchars_deep($_POST['telephone']);
		$email = htmlspecialchars_deep($_POST['email']);
		$username = htmlspecialchars_deep($_POST['username']);
	...
	case 'receive':
		$paycenter = htmlspecialchars_deep($_GET[paycenter]);
		$payonline_setting = get_pay_setting();
	...
	case 'exchange':
		...
		$begindate = htmlspecialchars_deep($_GET['begindate']);
		$enddate = htmlspecialchars_deep($_GET['enddate']);
	...
	case 'report':
		...
		showmsg('举报成功,谢谢您的参与！', trim(htmlspecialchars_deep($_SERVER['HTTP_REFERER'])));
	break;
	...
	case 'comment':
		...
		$userid = $_userid;
		$username = $_username;
		$content = htmlspecialchars_deep(trim($_REQUEST['content']));
		...
		$sql = "INSERT INTO {$table}comment (infoid,userid,username,postdate,is_check,ip,content) VALUES ('$infoid','$userid','$username','$content','$postdate','$check','$ip')";
		$res = $db->query($sql);
	...
	case 'delete':
		$id = is_array($_REQUEST['id']) ? array_map('intval',$_REQUEST['id']) : array(intval($_REQUEST['id']));
		if(empty($id))showmsg('没有选择记录');
		foreach((array)$id as $val){
			$sql = "select userid from {$table}comment where id='$val' ";
			$userid = $db->getOne($sql);
			if($userid!=$_userid)showmsg('此信息不是您的账号发布的，无法操作','?act=info_comment');
			$db->query("DELETE FROM {$table}comment WHERE id IN ($val)");
